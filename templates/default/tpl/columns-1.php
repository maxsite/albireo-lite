...
<div class="flex flex-wrap">
    {@loop $DATA}
        <div class="{{ $columnClass }} w100-tablet mar30-b bor-gray100 bor-solid bor1 rounded3">
            ...
            {@isset $image_large}
            __ <a href="{{ $page_url }}">{{ hThumb($image_large, wh: $image_large_size ?? '800x600', attr: ['alt/title'=>$title, 'class' =>'rounded3-t hover-img-gray30']) }}</a>
            
            {@endisset}

            h1(t130 pad10-rl) <a class="color-inherit" href="{{ $page_url }}">{* $title *}</a>
            
            <div class="t80 t-gray500 dark:t-primary200 mar15-b pad10-rl">
                {@isset $date}
            ...
            {@isset $announce}
                <div class="t90 animation-fade pad10-rl">{{ $announce }}</div>
            {@endisset}

            __(mar20-t mar15-b pad10-rl) <a class="t90 icon-arrow-right" href="{{ $page_url }}">{@lang Read}</a>
        </div>
    {@endloop}
</div>
