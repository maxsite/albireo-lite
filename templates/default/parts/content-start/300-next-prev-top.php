...
// тоже, что и 30-next-prev.php только другая опция
// если нет занятой, то это файл конфигурации
// next-prev-top: [config-dir]order-my.php

// если есть запятая, то это slug записей «prev, next»
// если какой-то страницы нет, то указывается ведущяя или конечная запятая
// next-prev-top: demo/astronomy/a1, demo/astronomy/a3
// next-prev-top: , demo/astronomy/a3
// next-prev-top: demo/astronomy/a1,


// если тип записи system то не выводить
if (getPageData('type') == 'system') return;

$pageData = getVal('pageData');

// если нет поля next-prev-top, то выходим
if (empty($pageData['next-prev-top'])) return;

$order = $pageData['next-prev-top'] ?? false;
...
$rows = \Pdo\PdoQuery::fetchAll($db, "SELECT file, title, header, page_url, image_large FROM file_info WHERE slug = :slug AND draft = 0;", [':slug' => $next]);
...
$rows = \Pdo\PdoQuery::fetchAll($db, "SELECT file, title, header, page_url, image_large FROM file_info WHERE slug = :slug AND draft = 0;", [':slug' => $prev]);
...
echo tpl(data: $np, tpl: TPL_DIR . 'next-prev-top.php');
